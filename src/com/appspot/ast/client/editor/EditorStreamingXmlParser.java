...
/**
 * Copied from org.waveprotocol.wave.model.document.parser.StreamingXmlParser
 */
public class EditorStreamingXmlParser implements XmlPullParser {
  /**
...
//  private static final String piEnd = "?>";
  ...
  public EditorStreamingXmlParser(String xml) throws XmlParseException {
...
//        GWT.log("attribute " + attr.first + "='" + attr.second + "'");
...
//      processingInstruction();
  ...
  String processingInstruction() throws XmlParseException {
//    ensure(match(piStart), "Matching <?");
    ...
    whitespace();
    int start = buffer.getPosition();
    while (buffer.hasMore() && !buffer.startsWith(String.valueOf(closeAngle))) {
      buffer.advanceCodeUnit(1);
    }
    int end = buffer.getPosition();
...
//    } else
    if (buffer.startsWith(openAngleSlash)) {
      return endTag();
